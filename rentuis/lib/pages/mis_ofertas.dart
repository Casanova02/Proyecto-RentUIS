...
class MisOfertas extends StatefulWidget {
  ...
  MisOfertas({required this.userEmail, required this.ofertas, required this.solicitudID});
...
class _MisOfertasPageState extends State<MisOfertas> {
  ...
  Future<List<dynamic>> obtenerObjetosDesdeFirebase(List<dynamic> ofertas) async {
    ...
    for (var ofertaId in ofertas) {
      try {
        ...
        DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('items').doc(ofertaId).get();
  ...
  void _getUserId(userId) async {
    final QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('usuarios').where('email', isEqualTo: userId).get();
